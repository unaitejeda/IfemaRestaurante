...
class ProductoDAO
{
    ...
    public static function crearPedido($id_usuario, $fecha, $total, $session, $propina, $esPedido)
{
    $con = DataBase::connect();
    $stmt = $con->prepare("INSERT INTO pedidos (id_usuario, hora, total, propina, pedido) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("isdii", $id_usuario, $fecha, $total, $propina, $esPedido); // Añadido el parámetro esPedido
    $stmt->execute();
